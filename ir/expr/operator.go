...
func (b *IR_Operator) SSA_Transform(ctx *SSA_Context) (SSA_Rewrites, IRExpression) {
	if IsLiteralOrVariable(b.Op1) {
		if IsLiteralOrVariable(b.Op2) {
			...
			return rewrites, NewIR_Operator(b.Opcode, b.Repr, b.Op1, NewIR_Variable(v))
		}
	} else {
		...
		rewrites = append(rewrites, NewSSA_Rewrite(v, expr))
		if IsLiteralOrVariable(b.Op2) {
			return rewrites, NewIR_Operator(b.Opcode, b.Repr, NewIR_Variable(v), b.Op2)
		} else {
			...
			return rewrites, NewIR_Operator(b.Opcode, b.Repr, NewIR_Variable(v), NewIR_Variable(v2))
		}
