...
func (i *IR_Mul) Encode(ctx *IR_Context, target encoding.Operand) ([]lib.Instruction, error) {
	...
	if returnType1 == TUint64 || returnType1 == TUint8 {
		...
		// TODO in 64 bit mode RDX should also be preserved
		...
		if shouldPreserveRdx {
			...
			for v, vTarget := range ctxCopy.VariableMap {
				if r, ok := vTarget.(*encoding.Register); ok && r.Register == 2 {
					fmt.Println("replacing", v, "with", tmpRdx)
		...
		if shouldPreserveRax {
			...
			for v, vTarget := range ctxCopy.VariableMap {
				if r, ok := vTarget.(*encoding.Register); ok && r.Register == 0 {
					fmt.Println("replacing", v, "with", tmpRax, raxInUse)
					ctxCopy.VariableMap[v] = tmpRax
				}
			}
		}
		...
		// TODO encode with a replaced VariableMap (changing variables that map to %rax)
