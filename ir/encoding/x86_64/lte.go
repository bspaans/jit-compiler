...
func encode_IR_LTE(i *expr.IR_LTE, ctx *IR_Context, target encoding.Operand, includeSETE bool) ([]lib.Instruction, error) {
	...
	if includeSETE {
		...
		// TODO xor tmpreg
		sete := x86_64.SETBE(tmpReg.Get8BitRegister())
		if IsSignedInteger(i.Op1.ReturnType(ctx)) {
			sete = x86_64.SETLE(tmpReg.Get8BitRegister())
		}
